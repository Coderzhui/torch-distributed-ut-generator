# -*- coding: utf-8 -*-
"""
测试目的：验证 torch.ao.quantization.observer.default_weight_observer 接口功能正确性
API 名称：torch.ao.quantization.observer.default_weight_observer
API 签名：default_weight_observer = MinMaxObserver.with_args(dtype=torch.qint8, qscheme=torch.per_tensor_symmetric)

覆盖维度表：
| 覆盖维度         | 说明                                                         | 覆盖情况                                       |
|------------------|--------------------------------------------------------------|------------------------------------------------|
| 空/非空          | 返回值为 .with_args 修饰后的可调用对象                       | 已覆盖                                         |
| 枚举选项         | dtype=qint8, qscheme=per_tensor_symmetric                   | 已覆盖                                         |
| 参数类型         | 无参数调用，返回 MinMaxObserver 子类实例                     | 已覆盖                                         |
| 传参与不传参     | default_weight_observer() 无参调用                           | 已覆盖                                         |
...
class TestDefaultWeightObserver(TestCase):
    ...
    def test_type(self):
        """Verify default_weight_observer is callable (a .with_args variant of MinMaxObserver)."""
        self.assertTrue(callable(default_weight_observer))

    def test_instantiation(self):
        """Verify that calling default_weight_observer() returns an nn.Module instance."""
    ...
    def test_configured_qscheme(self):
        """Verify the configured qscheme is per_tensor_symmetric."""
        observer = default_weight_observer()
