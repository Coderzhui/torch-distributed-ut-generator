# -*- coding: utf-8 -*-
"""
测试目的：验证 torch.ao.quantization.observer.default_float_qparams_observer 接口功能正确性
API 名称：torch.ao.quantization.observer.default_float_qparams_observer
API 签名：default_float_qparams_observer = PerChannelMinMaxObserver.with_args(dtype=torch.quint8, qscheme=torch.per_channel_affine_float_qparams, ch_axis=0)

覆盖维度表：
| 覆盖维度         | 说明                                                         | 覆盖情况                                       |
|------------------|--------------------------------------------------------------|------------------------------------------------|
| 空/非空          | 返回值为 .with_args 修饰后的可调用对象                       | 已覆盖                                         |
| 枚举选项         | dtype=quint8, qscheme=per_channel_affine_float_qparams       | 已覆盖                                         |
| 参数类型         | 无参数调用，返回 PerChannelMinMaxObserver 子类实例           | 已覆盖                                         |
| 传参与不传参     | default_float_qparams_observer() 无参调用                    | 已覆盖                                         |
...
class TestDefaultFloatQParamsObserver(TestCase):
    ...
    def test_type(self):
        """Verify default_float_qparams_observer is callable (a .with_args variant)."""
        self.assertTrue(callable(default_float_qparams_observer))

    def test_instantiation(self):
        """Verify that calling default_float_qparams_observer() returns an nn.Module instance."""
        observer = default_float_qparams_observer()
    ...
    def test_configured_qscheme(self):
        """Verify the configured qscheme is per_channel_affine_float_qparams."""
        observer = default_float_qparams_observer()
        self.assertEqual(observer.qscheme, torch.per_channel_affine_float_qparams)
